...
class ReleaseClient {
    ...
    async submitForReview(packageName, versionName, options) {
        ...
        const releases = await this._addRelease(packageName, useOptions.track, editId, Object.assign({ name: versionName, status: 'completed', releaseNotes: notes.length > 0 ? notes : [{ language: 'en-US', text: `New features for ${versionName}` }] }, optionals));
    ...
    async _addRelease(packageName, trackId, editId, release) {
        const existingReleases = await this.getReleases(packageName, trackId, editId);
        const releases = [];
        let didOverwrite = false;
        for (const existingRelease of existingReleases) {
            if (existingRelease.name === release.name) {
                releases.push(release);
                didOverwrite = true;
            }
            else {
                const pushRelease = existingRelease;
                if (pushRelease.status === 'completed') {
                    pushRelease.status = 'halted';
                    pushRelease.userFraction = 0;
                }
                releases.push(pushRelease);
            }
        }
        if (!didOverwrite) {
            releases.push(release);
