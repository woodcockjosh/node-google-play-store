...
export class ReleaseClient implements ReleaseClientInterface {
    ...
    public async submitForReview(packageName: string, versionName: string, options?: SubmitForReviewOptions): Promise<void> {
        ...
        const releases = await this._addRelease(packageName, useOptions.track, editId, {
            name: versionName,
            status: 'completed',
    ...
    private async _addRelease(packageName, trackId: TrackId, editId: string, release: androidpublisher_v3.Schema$TrackRelease): Promise<androidpublisher_v3.Schema$TrackRelease[]> {
